"""Tests for cgsbc.generate — integration and unit."""
...
@patch("cgsbc.generate.download_json", side_effect=_mock_download_json)
def test_main_produces_valid_json(mock_dl):
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = str(Path(tmpdir) / "output.json")
        with patch("sys.argv", ["cgsbc-generate", "--out", out_path]):
            main()

        with open(out_path, encoding="utf-8") as f:
            data = json.load(f)

    assert data["project"] == "ControlGate Security Baseline Catalog (CGSBC)"
...
def test_non_negotiable_high_only(mock_dl):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = str(Path(tmpdir) / "output.json")
        with patch("sys.argv", ["cgsbc-generate", "--out", out_path, "--non_negotiable_min_baseline", "high"]):
            main()
...
def test_download_json():
    ...
    with patch("cgsbc.generate.requests.get", return_value=mock_resp) as mock_get:
...
def test_main_module_importable():
    import cgsbc.__main__  # noqa: F401


def test_version_is_string():
    from cgsbc import __version__
